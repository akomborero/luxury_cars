...
export default function CarDetails({ carId }: CarDetailsProps) {
  ...
  <Image 
    ...
    alt={car.model} 
    fill 
    className="object-cover h-auto w-full" // Added h-auto
    unoptimized
  />
</div>
        ...
        <div className="flex flex-col">
          ...
          <div className="mt-auto pt-8 border-t border-gray-100 flex items-center justify-between">
            <div>
              <p className="text-gray-400 text-xs font-bold uppercase">Daily Rate</p>
              <p className="text-4xl font-black text-[#632197] italic">${car.price_per_day.toLocaleString()}</p>
            </div>
            <div className="mt-auto pt-8 border-t border-gray-100 flex items-center justify-between">
  ...
  <a 
    href={`https://wa.me/YOUR_PHONE_NUMBER?text=${encodeURIComponent(
      `Hi! I'm interested in booking the ${car.year} ${car.make} ${car.model}. Could you provide more details?`
    )}`}
    target="_blank"
    rel="noopener noreferrer"
    className="flex items-center gap-3 px-8 py-5 bg-[#25D366] text-white rounded-2xl font-black uppercase tracking-widest hover:bg-[#128C7E] transition-all shadow-lg active:scale-95"
  >
    {/* WhatsApp SVG Logo */}
    <svg 
      width="24" 
